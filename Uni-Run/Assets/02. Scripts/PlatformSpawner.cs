...
public class PlatformSpawner : MonoBehaviour
{
    ...
    public float timeBetSpawnMax = 2.25f; // 대음 배치까지의 시간 간격 최대값
    ...
    private GameObject[] plaftforms; // 미리 생성한 발판들
    ...
    void Start()
    {
        ...
        plaftforms = new GameObject[count];

        // count만큼 루프하면서 발판 생성
        for(int i = 0; i < count; i++)
        {
            // platformPrefab을 원본으로 새 발판을 poolPosition 위치에
            // 복제 생성. 생성된 발판을 platform 배열에 할당
            plaftforms[i] = Instantiate(platformPrefab, 
    ...
    void Update()
    {
        ...
        if(Time.time >= lastSpawnTime + timeBetSpawn)
        {
            ...
            // OnEnable 메서드가 실행됨
            plaftforms[currentIndex].SetActive(false);
            plaftforms[currentIndex].SetActive(true);

            // 현재 순번의 발판을 화면 오른쪽에 재배치
            plaftforms[currentIndex].transform.position =
